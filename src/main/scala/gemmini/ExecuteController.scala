...
class ExecuteController[T <: Data, U <: Data, V <: Data](xLen: Int, tagWidth: Int, config: GemminiArrayConfig[T, U, V])
  ...
  val do_computes_0 = DoComputes(0)
  val do_computes_1 = DoComputes(1)
  val do_computes_2 = DoComputes(2)

  val do_preloads_0 = DoPreloads(0)
  val do_preloads_1 = DoPreloads(1)
  val do_preloads_2 = DoPreloads(2)

  dontTouch(do_computes_0)
  dontTouch(do_computes_1)
  dontTouch(do_computes_2)
  dontTouch(do_preloads_0)
  dontTouch(do_preloads_1)
  dontTouch(do_preloads_2)
  ...
  dontTouch(dataBbank)
  dontTouch(dataDbank)
  dontTouch(dataABankAcc)
  dontTouch(dataBBankAcc)
  dontTouch(dataDBankAcc)
  ...
  dontTouch(dataA_unpadded)
  dontTouch(dataB_unpadded)
  dontTouch(dataD_unpadded)
  ...
  when (mesh_cntl_signals_q.io.deq.fire) {
    when (cntl.a_fire && mesh.io.a.fire && !cntl.a_garbage && cntl.a_unpadded_cols > 0.U) {
